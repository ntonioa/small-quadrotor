...
template <typename T>
static inline T my_clamp(T v, T lo, T hi) { return (v < lo) ? lo : (v > hi) ? hi : v; }
...
static inline int duty_from_unit(float u01) {
  ...
  if (ticks < 0) return 0;
  if (ticks > ticks_max) return ticks_max;
  return ticks;
}
...
static inline void motors_stop_all() {
  for (int i = 0; i < 4; ++i) ledcWrite(MOTOR_CHANNELS[i], 0);
}
// ======================================================================
...
namespace sym {
  ...
  struct Quaternion {
    ...
    static Quaternion FromStorage(const Eigen::Matrix<Scalar,4,1>& v) { Quaternion q; q.storage = v; return q; }
...
static constexpr float MASS = 0.070f;   // PATCH: 70 g reali
static constexpr float GRAV = 9.81f;

// PATCH: profilo throttle (puoi ritoccare)
static constexpr float HOVER_THR = 0.50f;     // stick di hover ~50%
static constexpr float THRUST_MAX_FACTOR = 1.0f; // full stick = 1.0×(m*g) tot

static Eigen::Matrix3f J_body() {
  Eigen::Matrix3f J;
  J << 3.55e-5f, 0.0f,     0.0f,
       0.0f,     4.23e-5f, 0.0f,
       0.0f,     0.0f,     7.47e-5f;
  return J;
}
static const Eigen::Vector3f kR(0.008f, 0.008f, 0.008f);
static const Eigen::Vector3f kW(0.008f, 0.008f, 0.008f);

// ====== Comandi da QGC ======
struct PilotCmd { float phi=0, theta=0, psi=0, thr=0; } g_cmd; // roll, pitch, yaw_ref, throttle(0..1)
struct StickScale {
  float max_roll  = 12.0f * M_PI/180.0f;  // prima 25°
  float max_pitch = 15.0f * M_PI/180.0f;  // prima 25°
  float max_yaw_rate = 45.0f * M_PI/180.0f; // prima 60°
} g_scale;
...
struct Mixer {
  float l = 0.062f;   // braccio [m]
  float km = 0.016f;  // coppia elica (tuning)
  Mat44 A;
  Mixer() {
    // X config: 1=FL, 2=FR, 3=RR, 4=RL
    A <<
      -1.0f/(2*l),  +1.0f/(2*l), -1.0f/(4*km),  0.25f,
      +1.0f/(2*l),  +1.0f/(2*l), +1.0f/(4*km),  0.25f,
      +1.0f/(2*l),  -1.0f/(2*l), -1.0f/(4*km),  0.25f,
      -1.0f/(2*l),  -1.0f/(2*l), +1.0f/(4*km),  0.25f;
    A.col(2) *= -1.0f; // flip yaw se necessario
  }
  Vec4 mix(const Vec4& y) const {
    Vec4 f = A * y;
    // PATCH: limiti coerenti al drone da 70 g
    const float F_tot_max = THRUST_MAX_FACTOR * MASS * GRAV;   // spinta massima totale
    const float fmax_motor = 0.30f * F_tot_max;                // ~30% del totale per motore
    const float fmin = 0.0f;
    for (int i=0; i<4; ++i) f(i) = std::clamp(f(i), fmin, fmax_motor);
    return f;
  }
} g_mixer;

// ====================== CURVA Forza <-> PWM (dati utente) =====================
// Dati (pwm 10..100) -> F [N]
// Fit lineare: F = m*pwm + b   (R^2 ~ 0.996)
//   m=0.00268140, b=-0.03073800
// Fit quadratico (più preciso): F = a*pwm^2 + b*pwm + c  (R^2 ~ 0.998)
//   a=4.8307e-06, b=0.00215003, c=-0.02011050
#ifndef FORCE_PWM_MODEL
#define FORCE_PWM_MODEL 1   // 1=lineare (robusto), 2=quadratico (più preciso)
#endif
static inline float clamp_pwm_range(float pwm){ return std::max(10.0f, std::min(100.0f, pwm)); }
static inline float pwm_to_u01(float pwm){ float u=(pwm-10.0f)/90.0f; return std::max(0.0f,std::min(1.0f,u)); }
static inline float force_to_pwm_linear(float F){ const float m=0.00268140f, b=-0.03073800f; return clamp_pwm_range((F - b)/m); }
static inline float force_to_pwm_quad  (float F){ const float a=4.8307e-06f,b=0.00215003f,c=-0.02011050f; float disc=b*b - 4*a*(c - F); if(disc<0) disc=0; return clamp_pwm_range((-b + sqrtf(disc))/(2*a)); }
static inline float force_to_u01(float F){ if(F<=0.0f) return 0.0f; float pwm = (FORCE_PWM_MODEL==2)? force_to_pwm_quad(F) : force_to_pwm_linear(F); return pwm_to_u01(pwm); }

// ===== N -> duty usando la curva calibrata =====
static inline void motors_from_forces_and_write(const Vec4& f) {
  for (int i=0;i<4;++i) {
    float Fi = std::max(0.0f, f(i));
    float u  = force_to_u01(Fi);  // Forza [N] -> duty 0..1
    write_motor_duty01(i, u);
  }
}
// ============================================================================
...
static void mav_send(const mavlink_message_t& msg) {
  ...
  udp.beginPacket(qgc_ip, QGC_PORT); udp.write(buf, len); udp.endPacket();
}
...
static void send_attitude_quat(float qw, float qx, float qy, float qz, float p, float q, float r) {
  mavlink_message_t msg; float cov[9]; for (int i=0;i<9;i++) cov[i] = NAN;
  mavlink_msg_attitude_quaternion_pack(sysid, compid, &msg, millis(), qw, qx, qy, qz, p, q, r, cov);
  mav_send(msg);
}

// ====== Sicurezza ======
// Per evitare che il drone parta in volo senza comandi, o con comandi
bool g_armed = false;
  ...
  for (int i = 0; i < n; ++i) {
    if (mavlink_parse_char(MAVLINK_COMM_0, rxbuf[i], &msg, &rx_status)) {
      ...
      if (msg.msgid == MAVLINK_MSG_ID_MANUAL_CONTROL) {
        ...
        // abbiamo un comando valido → aggiorna timestamp
        last_cmd_ms = millis();
        ...
        g_cmd.thr   =  thr_norm;
        // g_cmd.psi rimane come heading di riferimento (hold)
        ...
        if (yaw_norm > 0.9f && thr_norm < 0.05f) {
          if (state != 1) { state = 1; yaw_edge_ms = millis(); }
          if (millis() - yaw_edge_ms > 2000) arm_if_safe();
        } else if (yaw_norm < -0.9f && thr_norm < 0.05f) {
          if (state != -1) { state = -1; yaw_edge_ms = millis(); }
          if (millis() - yaw_edge_ms > 2000) disarm_all();
        } else {
          state = 0;
        }
      }
...
void calibrateAccelGyro(int samples = 500) {
  ...
  for(int i=0;i<samples;i++) {
    if(imu.dataReady()) {
      imu.getAGMT();
      a[0]+=imu.accX(); a[1]+=imu.accY(); a[2]+=imu.accZ()-1000.0f; // 1g
      g[0]+=imu.gyrX(); g[1]+=imu.gyrY(); g[2]+=imu.gyrZ();
      delay(5);
    }
  }
  for(int i=0;i<3;i++){ calibration.accelOffset[i]=a[i]/samples; calibration.gyroOffset[i]=g[i]/samples; }
...
void setup() {
  ...
  while(true) {
    ...
    if(imu.status == ICM_20948_Stat_Ok) { Serial.println("IMU ok"); break; }
    Serial.println("IMU fail, retry..."); delay(500);
  }
  imu.swReset(); delay(10); imu.sleep(false); imu.lowPower(false); imu.startupMagnetometer(); delay(100);
...
// ====== LOOP ======
void loop() {
  if (!imu.dataReady()) { qgc_parse_and_update_cmd(); delay(2); return; }
  ...
  if (acc_norm > 1e-6f) { accX/=acc_norm; accY/=acc_norm; accZ/=acc_norm; }

  // EKF predict/update
  // Vec3 z; z << accX, accY, accZ; // misura solo acc
  Vec3 z; 
  z << -accX, -accY, -accZ;   // <-- inverti qui

  Vec7 x_pred; Mat77 P_pred; Predict(x, dt, P, Qm, &x_pred, &P_pred);
  Vec7 x_upd;  Mat77 P_upd;  Update(x_pred, P_pred, z, Rm, &x_upd, &P_upd);
  x_upd.head<4>().normalize(); x = x_upd; P = P_upd;
  ...
  // Subito dopo aver calcolato roll/pitch/yaw dall’EKF:
  g_cmd.psi = yaw;   // <-- aggiornalo ad ogni ciclo (non solo all'inizio)
  ...
  if (now_ms - last_hb_ms >= 1000) { last_hb_ms = now_ms; send_heartbeat(); }
  if (now_ms - last_att_ms >= 20)   { last_att_ms = now_ms; send_attitude_quat(qw,qx,qy,qz,p_rate,q_rate,r_rate); send_attitude(roll,pitch,yaw,p_rate,q_rate,r_rate); }

  // Init yaw reference una volta
  static bool psi_init = false; if (!psi_init) { g_cmd.psi = yaw; psi_init = true; }

  // ===== Leggi comandi QGC =====
  qgc_parse_and_update_cmd();

    /// ===== Controller =====
  Eigen::Matrix<float,4,1> q_storage; q_storage << qw,qx,qy,qz;
  sym::Quaternion<float> q_sym = sym::Quaternion<float>::FromStorage(q_storage);
  Eigen::Vector3f omega_body(p_rate, q_rate, r_rate);

  // PATCH: 0..1 stick → 0..THRUST_MAX_FACTOR (hover a HOVER_THR)
  ...
  sym::AttitudeControllerStep<float>(
      ...
      kR, kW, J_body(),
      MASS, GRAV, thr_eff,   // PATCH: usa thr_eff
      &y_wrench);
    y_wrench(2) = 0.0f;   // <-- niente τ_z al mixer
  // ===== Safety layer per BRUSHED con MOSFET =====

  // 1) Failsafe segnale: se non ricevo MANUAL_CONTROL da >300 ms → disarma
  if (millis() - last_cmd_ms > 300) {
    if (g_armed) disarm_all();
  }

  // 2) Deadman su throttle: se throttle < 5% tengo i motori a 0
  const bool throttle_low = (g_cmd.thr < 0.05f);

  // 3) Se non armato o throttle basso → motori OFF e salta l'uscita
  if (!g_armed || throttle_low) {
    motors_stop_all();     // duty 0 su tutti i canali
    return;                // esci dal loop() qui
  }
  ...
  // 6) Slew-rate limit per evitare salti quando armi o correggi forte
  static Eigen::Vector4f f_prev = Eigen::Vector4f::Zero();
  const float F_tot_max = THRUST_MAX_FACTOR * MASS * GRAV;
  const float f_full = 0.30f * F_tot_max;        // per singolo motore (coerente col mixer)
  const float max_step = 0.15f * f_full;         // ~15% per ciclo
  for (int i=0;i<4;++i) {
    float df = f(i) - f_prev(i);
    if (df >  max_step) f(i) = f_prev(i) + max_step;
    if (df < -max_step) f(i) = f_prev(i) - max_step;
  }
  f_prev = f;

  // 7) Scrittura motori (N → duty 0..1 @78 kHz) via curva calibrata
