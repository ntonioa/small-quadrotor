...
void setup() {
  ...
  Serial.print("Mag status after startup: ");
  Serial.println(imu.status); // Deve essere 0!
  ...
  // Q.setIdentity(); Q *= 0.01f;
  // R.setIdentity(); R *= 0.1f;
  Q.setIdentity(); Q *= 0.1f;
  R.setIdentity(); R *= 1.0f;

  // Q.setIdentity();  Q *= 0.001f;
  // R.setIdentity();
  R.block<3,3>(0,0) *= 0.0f;   // gyro noise
  R.block<3,3>(3,3) *= 0.0f;   // mag noise
