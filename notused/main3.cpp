...
void loop() {
  if(imu.dataReady()) {
    ...
    // Applica correzione agli accelerometri
    ...
    // Applica correzione ai giroscopi
    ...
    // Applica correzione al magnetometro
    ...
    for(int i = 0; i < 3; i++) {
      ...
      smoothMag[i] = (1-filterFactor)*smoothMag[i] + filterFactor*((i==2) ? magZ : (i==1) ? magY : magX);
    }

    // Calcola Roll, Pitch e Yaw
    calculateOrientation(smoothAcc[0], smoothAcc[1], smoothAcc[2], smoothMag[0], smoothMag[1], smoothMag[2]);

    // Stampa dati
    Serial.print("Roll:"); Serial.print(orientation.roll, 1);
    Serial.print("° Pitch:"); Serial.print(orientation.pitch, 1);
