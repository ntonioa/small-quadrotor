...
# Variante con compensazione dell'inclinazione (tilt compensation):
# Per mantenere la stessa accelerazione verticale in world (es. hover),
# la spinta totale T richiede T ≈ m*g / (e3^T R^T e3) = m*g / R33, dove
# R33 = 1 - 2*(q.x^2 + q.y^2). Usiamo un floor per evitare divisioni instabili.
def thrust_from_throttle_tilt_comp(throttle, q: sm.Quaternion, m, g):
    # cos_tilt = R33 = 1 - 2*(x^2 + y^2) per convenzione quaternion [w,x,y,z]
    cos_tilt = 1 - 2*(q.x*q.x + q.y*q.y)
    safe = sm.max(1e-3, cos_tilt)  # evita cos_tilt ~ 0 o negativo
    return sm.Matrix([m*g*throttle / safe])
...
# -------------------------------------------------------
# 3b) thrust_from_throttle_tilt_comp: (throttle, q, m, g) -> Fz (Scalar 1x1)
#     Compensa roll/pitch per mantenere la stessa componente verticale.
# -------------------------------------------------------
Codegen.function(
    func=lambda thr,q_in,m_in,g_in: thrust_from_throttle_tilt_comp(thr, q_in, m_in, g_in),
    input_types=[throttle, q, m, g],
    output_names=["Fz"],
    name="thrust_from_throttle_tilt_comp",
    config=CppConfig(),
).generate_function("generated")
...
# Variante: step con compensazione dell'inclinazione sulla spinta
def attitude_controller_step_tiltcomp(q, omega, phi, theta, psi, kR, kW, J, m, g, throttle):
    qd_storage = q_from_rpy_storage(phi, theta, psi)
    qd = sm.Quaternion.from_storage(qd_storage)
    tau = tau_att(q, qd, omega, kR, kW, J)
    Fz  = thrust_from_throttle_tilt_comp(throttle, q, m, g)
    return make_wrench(tau, Fz)
...
# Step alternativo con tilt compensation
Codegen.function(
    func=lambda q,omega,phi,theta,psi,kR,kW,J,m,g,thr: sm.Matrix(
        attitude_controller_step_tiltcomp(q,omega,phi,theta,psi,kR,kW,J,m,g,thr)
    ),
    input_types=[q, omega, phi_d, theta_d, psi_d, kR, kW, J, m, g, throttle],
    output_names=["y"],
    name="attitude_controller_step_tiltcomp",
    config=CppConfig(),
).generate_function("generated")
